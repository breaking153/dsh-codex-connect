...
function parseProfilesDocument(value) {
	...
	if (rawAccounts !== void 0 && (typeof rawAccounts !== "object" || rawAccounts === null || Array.isArray(rawAccounts))) return void 0;
	const accounts = {};
...
/** Current on-disk format; version 1 remains readable for migration. */
...
var OpenAICodexCredentialStore = class {
	...
	*/
	...
	async snapshot() {
		const document = await this.readDocument();
		return document === void 0 ? void 0 : {
			activeAccountId: document.activeAccountId,
			accountIds: document.accounts.map((account) => account.accountId)
		};
	}
	...
	*/
	forAccount(accountId) {
		...
		return {
			async read(providerId) {
				...
				return account === void 0 ? void 0 : structuredClone(account);
			},
			async list() {
			...
			async modify(providerId, fn) {
				...
				await mkdir(dirname(owner.filename), {
					recursive: true,
					mode: 448
				});
				return withFileLock(owner.filename, async () => {
					...
					const current = index < 0 ? void 0 : document.accounts[index];
					...
					if (document === void 0 || index < 0) throw new Error("openai-codex: account was not found");
	...
	async activateIfActive(expectedAccountId, nextAccountId) {
		...
		return withFileLock(this.filename, async () => {
			const document = await this.readDocument();
			if (document === void 0) return { activated: false };
			if (document.activeAccountId !== expectedAccountId) return {
				activated: false,
				activeAccountId: document.activeAccountId
			};
			if (!document.accounts.some((account) => account.accountId === nextAccountId)) return {
				activated: false,
				activeAccountId: document.activeAccountId
			};
			if (nextAccountId !== expectedAccountId) await this.writeDocument({
				...document,
	...
	async modify(providerId, fn) {
		...
		return withFileLock(this.filename, async () => {
			...
			const accounts = document?.accounts.map((account) => structuredClone(account)) ?? [];
			const existing = accounts.findIndex((account) => account.accountId === validated.accountId);
			if (existing === -1) accounts.push(validated);
			else accounts[existing] = validated;
...
const TERMINAL_QUOTA_CODES = /* @__PURE__ */ new Set([
	"usage_limit_reached",
	"usage_not_included",
	"insufficient_quota"
]);
const DEDUPLICATION_LIMIT = 4096;
const ERROR_RESPONSE_LIMIT = 65536;
/** Accept only a machine-code allowlist, never a generic status or prose match. */
function openAICodexTerminalQuotaCode(detail) {
...
function isOpenAICodexTerminalQuotaError(detail) {
	return openAICodexTerminalQuotaCode(detail) !== void 0;
}
async function boundedResponseText(response) {
	if (!response.headers.get("content-type")?.toLowerCase().includes("json")) return void 0;
	const declared = Number(response.headers.get("content-length"));
	if (Number.isFinite(declared) && declared > ERROR_RESPONSE_LIMIT) return void 0;
...
async function terminalQuotaCodeFromResponse(response) {
	if (response.ok) return void 0;
	try {
		const text = await boundedResponseText(response);
		return text === void 0 ? void 0 : openAICodexTerminalQuotaCode(text);
	} catch {
		return;
	}
}
...
function combinedMessage(previous, current, content = current.content) {
...
function canStillMatchSuffix(prefix, candidate) {
...
var ExactPrefixDeduplicator = class {
	prefix;
	buffered = "";
	decided = false;
...
function setupFailure(model, error) {
	return {
		...
		usage: emptyUsage(),
		stopReason: "error",
		errorMessage: error instanceof Error ? error.message : "OpenAI Codex fallback setup failed",
		timestamp: Date.now()
	};
}
function orderedCandidates(accountIds, activeAccountId) {
...
function withOpenAICodexAccountFallback(provider, credentials, resolveAccessToken, enabled = () => false, notify) {
	...
	return {
		...
		streamSimple(model, originalContext, options) {
			...
			const pump = async () => {
				...
				const initialAccountId = initialAccess === void 0 ? void 0 : await credentials.accountIdForAccessToken(initialAccess);
				if (snapshot === void 0 || initialAccountId === void 0 || snapshot.activeAccountId !== initialAccountId) {
				...
				const attempted = /* @__PURE__ */ new Set([initialAccountId]);
				let previous;
				...
				let firstQuotaCode;
				let firstQuotaError;
				const rollback = async (reasonCode) => {
					...
					emit({
						...
						toAccountId: initialAccountId,
						reasonCode,
						attempt,
						rollbackApplied: result.activated
					});
					fallbackActive = false;
				};
				while (true) {
					const offset = previous?.content.length ?? 0;
					const deduplicator = new ExactPrefixDeduplicator(previous === void 0 ? "" : visibleText(previous));
					...
					try {
						for await (const event of inner) {
							...
							if (event.type === "done" || event.type === "error") {
								...
								if (released !== "" && firstTextIndex !== void 0) {
									firstText += released;
									output.push({
										type: "text_delta",
										contentIndex: firstTextIndex + offset,
										delta: released,
										partial: withCombinedPartial(previous, raw, firstTextIndex, firstText)
									});
								}
								...
								const quotaCode = event.type === "error" && event.reason === "error" ? responseQuotaCode ?? openAICodexTerminalQuotaCode(event.error.errorMessage ?? "") : void 0;
								if (quotaCode !== void 0 && options?.signal?.aborted !== true && !hasToolCall(raw) && ![...openBlocks.values()].includes("toolCall")) {
									firstQuotaCode ??= quotaCode;
									firstQuotaError ??= event.type === "error" ? event.error.errorMessage : void 0;
									let selected = false;
									for (const candidate of candidates) {
										...
										if (candidateAccess === void 0 || await credentials.accountIdForAccessToken(candidateAccess) !== candidate) {
											emit({
												type: "candidate-rejected",
												sessionId: options?.sessionId,
												fromAccountId: currentAccountId,
										...
										if (!(await credentials.activateIfActive(currentAccountId, candidate)).activated) {
											emit({
												type: "superseded",
												sessionId: options?.sessionId,
												fromAccountId: currentAccountId,
												toAccountId: candidate,
												reasonCode: quotaCode,
												attempt
											});
											if (firstQuotaError !== void 0) combined = {
												...combined,
												errorMessage: firstQuotaError
											};
											output.push({
												type: "error",
												reason: "error",
										...
										for (const [index, type] of openBlocks) {
											...
											if (type === "thinking" && block?.type === "thinking") output.push({
												type: "thinking_end",
												contentIndex: index + offset,
									...
									emit({
										...
										reasonCode: firstQuotaCode ?? quotaCode,
										attempt
									});
									if (firstQuotaError !== void 0) combined = {
										...combined,
								...
								if (event.type === "done") {
									if (fallbackActive) emit({
										type: "completed",
										sessionId: options?.sessionId,
										fromAccountId: initialAccountId,
										toAccountId: currentAccountId,
										reasonCode: firstQuotaCode ?? "terminal_quota",
										attempt
									});
									output.push({
										...event,
							...
							if (event.type === "text_delta") {
								...
								if (delta !== "") output.push(remapEvent({
									...event,
									delta
...
*/
function createOpenAICodexAdapter(credentials, resolveAttachments, fastMode, visibleModelIds, proxyManager, resolveProxyUrl, contextWindowOverrides, enableAccountFallback, onAccountFallback) {
	const provider = openaiCodexProvider();
	const accountModels = /* @__PURE__ */ new Map();
	const resolveAccountAccessToken = async (store, accountId) => {
	...
	const currentProfiles = () => {
		...
		if (profiles === void 0 || !deepEqualJson(previousOverrides, overrides)) {
			...
			const profile = enableAccountFallback === void 0 ? baseProfile : {
				...baseProfile,
				piProvider: withOpenAICodexAccountFallback(baseProfile.piProvider, credentials, resolveAccountAccessToken, enableAccountFallback, onAccountFallback)
			};
...
/** Enumerate saved accounts or explicitly switch the active account. */
...
function registerOpenAICodexAuthRoutes(ctx, store, trustedOriginsOverride, fastModeOverride, proxyManager, resolveProxyUrl, authorizationTimeoutMs) {
	...
	ctx.effect(() => {
		const stateWithAccounts = async (status) => ({
			...status,
		...
		const routes = [
			ctx.webServer.register({
				...
				handler: async (req, res) => {
					...
					json$4(res, 200, await stateWithAccounts(await auth.status()));
				}
			}),
			ctx.webServer.register({
				kind: "exact",
			...
			ctx.webServer.register({
				...
				handler: async (req, res) => {
					...
					if (!await authorize(req, res)) return;
					try {
						json$4(res, 200, await stateWithAccounts(await auth.cancel()));
					} catch (error) {
						json$4(res, 500, { error: safeMessage(error) });
					}
				}
			...
			ctx.webServer.register({
				...
				handler: async (req, res) => {
					...
					if (!await authorize(req, res)) return;
					try {
						json$4(res, 200, await stateWithAccounts(await auth.signOut()));
					} catch (error) {
						json$4(res, 500, { error: safeMessage(error) });
					}
				}
			...
			ctx.webServer.register({
				...
				handler: async (req, res) => {
					...
					try {
						...
						return json$4(res, 200, await stateWithAccounts(status));
					} catch (error) {
...
//#endregion
export { OPENAI_CODEX_PROXY_PROBE_TIMEOUT_MS as $, openAICodexConflictMessage as A, openAICodexModelCatalog as At, DSH_PLUGIN_API_PACKAGES as B, resolveOpenAICodexAccountProfiles as Bt, OpenAICodexTransport as C, normalizeTrustedOrigin as Ct, IMAGE_GENERATE_TOOL_NAME as D, loginOpenAICodex as Dt, VIEW_IMAGE_TOOL_NAME as E, readOpenAICodexRateLimits as Et, parseOpenAICodexUpdateResult as F, OPENAI_CODEX_PROVIDER as Ft, assessCompatibility as G, SUPPORTED_DSH_PLUGIN_API_VERSION as H, parseOpenAICodexVersion as I, OpenAICodexCredentialStore as It, readInstalledPackageVersion as J, detectCompatibility as K, COMPATIBILITY_CONTRACT as L, openAICodexAuthPath as Lt, OPENAI_CODEX_UPDATE_PATH as M, openAICodexTerminalQuotaCode as Mt, checkForOpenAICodexUpdate as N, withOpenAICodexAccountFallback as Nt, assertNoOpenAICodexProviderConflict as O, logoutOpenAICodex as Ot, compareOpenAICodexVersions as P, OPENAI_CODEX_AUTH_FILENAME as Pt, OPENAI_CODEX_PROXY_CANDIDATE_LIMIT as Q, COMPATIBILITY_PACKAGES as R, OPENAI_CODEX_ACCOUNT_PROFILES_FILENAME as Rt, OPENAI_CODEX_TRANSPORT_SERVICE as S, OpenAICodexTrustedOriginsStore as St, isOpenAICodexTransportError as T, parseOpenAICodexUsage as Tt, SUPPORTED_NODE_RANGE as U, PI_AI_PACKAGE as V, SUPPORTED_PI_AI_RANGE as W, OPENAI_CODEX_PROXY_TEST_PATH as X, OPENAI_CODEX_PROXY_DETECT_PATH as Y, OPENAI_CODEX_LOCAL_PROXY_CANDIDATES as Z, OPENAI_CODEX_IMAGE_MAX_RESPONSE_BYTES as _, OPENAI_CODEX_FAST_MODE_PATH as _t, name as a, DEFAULT_OPENAI_CODEX_SEARCH_CONTEXT_SIZE as at, OPENAI_CODEX_TRANSPORT_API_VERSION as b, OPENAI_CODEX_FAST_MODE_MAX_SESSION_ID_LENGTH as bt, migrateOpenAICodexSearchHistory as c, DEFAULT_OPENAI_CODEX_SEARCH_MODEL as ct, OPENAI_CODEX_SEARCH_URL as d, decodeOpenAICodexSettings as dt, OPENAI_CODEX_PROXY_PROBE_URL as et, OpenAICodexSearchProvider as f, isValidOpenAICodexContextWindowOverrides as ft, OPENAI_CODEX_IMAGE_MAX_ERROR_BYTES as g, resolveOpenAICodexSettings as gt, OPENAI_CODEX_IMAGE_MAX_COUNT as h, resolveOpenAICodexProxyUrl as ht, inject as i, DEFAULT_OPENAI_CODEX_PROXY_URL as it, CODEX_CONNECT_VERSION as j, isOpenAICodexTerminalQuotaError as jt, diagnoseOpenAICodex as k, openAICodexAuthStatus as kt, OPENAI_CODEX_BASE_URL as l, DEFAULT_OPENAI_CODEX_SETTINGS as lt, OPENAI_CODEX_IMAGE_GENERATION_URL as m, normalizeOpenAICodexProxyUrl as mt, OPENAI_CODEX_SETTINGS_NS as n, detectOpenAICodexProxies as nt, OPENAI_CODEX_HISTORY_BACKUP_SUFFIX as o, DEFAULT_OPENAI_CODEX_SEARCH_MAX_OUTPUT_TOKENS as ot, mapOpenAICodexSearchResponse as p, isValidOpenAICodexProxyUrl as pt, evaluateCompatibility as q, apply as r, listOpenAICodexProxyCandidates as rt, OPENAI_CODEX_SEARCH_MODEL_REQUEST_EVENT as s, DEFAULT_OPENAI_CODEX_SEARCH_MODE as st, Config as t, OpenAICodexProxyManager as tt, OPENAI_CODEX_SEARCH_PROVIDER as u, OPENAI_CODEX_SETTINGS_NAMESPACE as ut, OPENAI_CODEX_IMAGE_PROMPT_MAX_LENGTH as v, FastModeRegistry as vt, OpenAICodexTransportError as w, OPENAI_CODEX_USAGE_URL as wt, OPENAI_CODEX_TRANSPORT_ERROR_CODES as x, isFastModeSessionId as xt, OPENAI_CODEX_IMAGE_REQUEST_TIMEOUT_MS as y, OPENAI_CODEX_FAST_MODE_MAX_SESSIONS as yt, COMPATIBILITY_SCHEMA_VERSION as z, openAICodexAccountProfilesPath as zt };
