	...
	factory: (require) => {
		...
		*/
		...
		var OpenAICodexAccountStore = class {
			...
			async cancel() {
				...
				try {
					const response = await request(OPENAI_CODEX_AUTH_CANCEL_PATH, "POST");
					this.publish({
						...this.splitServerState(response),
						busy: false
					});
				} catch (error) {
					this.publish({
			...
			async signOut() {
				...
				try {
					const response = await request(OPENAI_CODEX_AUTH_LOGOUT_PATH, "POST");
					this.publish({
						...this.splitServerState(response),
						busy: false
					});
			...
			async activate(accountId) {
				...
				try {
					const response = await request(OPENAI_CODEX_AUTH_ACCOUNTS_PATH, "POST", void 0, { accountId });
					this.publish({
						...this.splitServerState(response),
						busy: false
					});
		...
		*/
		...
		const en = {
			...
			activeAccountHelp: "New Codex requests use the selected account.",
			enableAccountFallback: "Automatic quota fallback",
			enableAccountFallbackHelp: "If a request ends with a confirmed quota code before any tool starts, continue on the next saved account. Account switches are recorded without tokens or provider error text.",
			enableAccountFallbackNeedsAccounts: "Add at least two accounts to enable automatic fallback.",
			accountFallbackSaveFailed: "Automatic fallback could not be updated.",
		...
		const zh = {
			...
			addAccount: "添加账户",
			savedAccounts: "已保存账户",
			activeAccountHelp: "新的 Codex 请求将使用所选账户。",
			enableAccountFallback: "自动额度 Fallback",
			enableAccountFallbackHelp: "仅当请求在工具开始前以已确认的额度机器码结束时，才使用下一个已保存账户继续；切换记录不包含令牌或服务端错误原文。",
			enableAccountFallbackNeedsAccounts: "至少添加两个账户后才能启用自动 Fallback。",
			accountFallbackSaveFailed: "无法更新自动 Fallback 设置。",
			moreSettings: "更多设置",
		...
		*/
		...
		function AccountFallbackToggle({ t, configScope, accountCount }) {
			const subscribe = (0, react.useCallback)((listener) => configScope?.subscribe(listener) ?? (() => void 0), [configScope]);
			const getSnapshot = (0, react.useCallback)(() => configScope?.getSnapshot() ?? UNAVAILABLE_CONFIG, [configScope]);
			const snapshot = (0, react.useSyncExternalStore)(subscribe, getSnapshot, getSnapshot);
			const [busy, setBusy] = (0, react.useState)(false);
			const [failed, setFailed] = (0, react.useState)(false);
			if (configScope === void 0 || snapshot.value === void 0) return null;
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
					...
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
						...
						checked: enabled,
						disabled: !editable,
						"aria-label": t("enableAccountFallback"),
						onChange: (event) => {
							const next = event.currentTarget.checked;
							setBusy(true);
							setFailed(false);
							configScope.set("enableAccountFallback", next).catch(() => {
								setFailed(true);
							}).finally(() => {
								setBusy(false);
							});
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							...
							children: t("enableAccountFallback")
						}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							style: secondaryStyle,
							children: accountCount > 1 ? t("enableAccountFallbackHelp") : t("enableAccountFallbackNeedsAccounts")
						})]
					})]
					...
					children: t("accountFallbackSaveFailed")
				}) : null]
			});
		}
		...
		function OpenAICodexModelsCard({ t, account, configScope }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					status.status === "signed-in" && accounts.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						style: {
							display: "flex",
							alignItems: "center",
							flexWrap: "wrap",
						...
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("label", {
								htmlFor: `${detailsId}-account`,
								style: {
									...secondaryStyle,
									fontWeight: 500
								},
								children: t("savedAccounts")
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("select", {
								id: `${detailsId}-account`,
								"aria-label": t("savedAccounts"),
								style: accountSelectStyle,
								value: activeAccount?.accountId ?? "",
								disabled: snapshot.busy,
								onChange: (event) => {
									account.activate(event.currentTarget.value);
								},
								children: accounts.map((saved) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
									value: saved.accountId,
									children: saved.email === void 0 ? saved.displayName : `${saved.displayName} · ${saved.email}`
								}, saved.accountId))
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								style: secondaryStyle,
					...
					status.status === "signed-in" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(AccountFallbackToggle, {
						t,
