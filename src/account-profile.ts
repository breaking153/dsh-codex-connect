...
function parseProfilesDocument(value: unknown): AccountProfilesDocument | undefined {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) return undefined
  const record = value as Record<string, unknown>
  if (record['version'] !== 1 || Object.keys(record).some(key => !['version', 'localUsername', 'accounts'].includes(key))) return undefined
  const localUsername = boundedText(record['localUsername'], MAX_LABEL_LENGTH)
  const rawAccounts = record['accounts']
  if (rawAccounts !== undefined && (typeof rawAccounts !== 'object' || rawAccounts === null || Array.isArray(rawAccounts))) return undefined
