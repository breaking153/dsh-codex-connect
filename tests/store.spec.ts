...
describe('OpenAICodexCredentialStore', () => {
  ...
  it('keeps multiple OAuth accounts, switches explicitly, and removes only the active one', async () => {
    const auth = await store()
    ...
    await expect(auth.activate('missing-account')).rejects.toThrow(/not found/u)
