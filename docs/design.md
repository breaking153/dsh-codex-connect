...
The plugin uses `$DSH_HOME/.openai-codex-auth.json`, separate from Codex CLI/Desktop state. Version 2 stores one explicit `activeAccountId` and a set of OAuth credentials. The strict reader normalizes legacy version-1 single-account documents in memory and the next successful credential write migrates them. POSIX reads reject group/world-accessible files. Parent directories and files are created with owner-only modes, writes are atomic, and every mutation uses the Harness cross-process file lock. Callers receive cloned credentials. The browser receives only local, non-secret account summaries and can list or explicitly activate a saved account. Automatic account fallback is opt-in, is limited to confirmed terminal quota exhaustion before any tool call, and records redacted account switches. Browser-origin trust is a separate `$DSH_HOME/.openai-codex-trusted-origins.json` sidecar with `version: 1`, `mode: "allowlist"`, and exact normalized HTTP(S) origins; it never contains OAuth material and is changed only by the standalone CLI.
